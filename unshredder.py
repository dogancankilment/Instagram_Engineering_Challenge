...
def unshred(image, shred_width):
    ...
    def unshred_islemi(yerlestirme, shreds_left, unshred_order):
        ...
        unshred order listesinin basina eklenmeli. Bu sayede liste basina ekledigimiz yeni parca
        gecerli olan unshredded resmimizin sag parcasi olacaktir.

        Eger baslangicta unshred order listemiz bos ise yerlestirmeye baslarken en dusuk degerli olan
        parcayi alip, cift tarafli bakarak yerlestirmeye devam etmeliyiz.
        ...
        if shreds_left == 0:
            ...
            if not unshred_order: #liste ilk adimda bos ise
                ...
                if best_prepend and best_append and best_prepend[0][2] < best_append[0][2] or best_prepend:
                ...
                unshred_order_next[-1] : unshred_order_next'in son elemani - sag parca gibi

                """

            yerlestirme_next = filter(lambda x: x[0] != unshred_order_next[0] and x[1] != unshred_order_next[-1], yerlestirme)
    ...
    # Eger geri donen deger negatif ise if x < y, sifir ise if x == y ve tam anlamiyla pozitif ise if x > y.
    yerlestirme = sorted([(sol, sag, pixel_islemleri(image, shred_width, sol, sag)) for sol in xrange(shreds) for sag in xrange(shreds) if sol != sag], cmp=lambda x, y: x[2] - y[2])

    return unshred_islemi(yerlestirme, shreds, [])


if __name__ == "__main__":
    image = PIL.Image.open(sys.argv[1]) # kullanici girdi olarak yazdigi resmi isleme tabi tutacagiz
